  ...
  return (
    ...
    <article className="animate-fade-up overflow-hidden rounded-2xl border border-border bg-surface">
      ...
      {post.media.length > 0 && (
        ...
        <div className={`grid gap-1 px-4 pb-3 ${post.media.length === 1 ? "" : "grid-cols-2"}`}>
          ...
          {post.media.slice(0, 4).map((m, i) => (
            ...
            <div key={i} className="relative overflow-hidden rounded-xl border border-border bg-background">
              ...
              {m.type === "video" ? (
                ...
                <a href={m.url} target="_blank" rel="noreferrer" className="flex aspect-[4/3] flex-col items-center justify-center gap-2 p-4 text-center text-xs text-muted-foreground hover:text-vsm-red">
